...
module.exports.attributeCount = function*(parent, data, subcategory) {
    ...
    for (var i = 0; i < subcategory.length; i++) {
        ...
        for (var j = 0; j < data.length; j++) {
            if (data[j] == "Brand") {
              x = data[j].replace(" ","").toLowerCase() + ".org";
              console.log("x is :",x);
    ...
    for(var i=0;i<allbuckets.length;i++) {
      for(var j=i+1;j<allbuckets.length;j++) {
        if(allbuckets[i].key == allbuckets[j].key) {
          allbuckets[i]["doc_count"] = allbuckets[i]["doc_count"] + allbuckets[j]["doc_count"];
          allbuckets.splice(j,1);
        }
      }
    }
