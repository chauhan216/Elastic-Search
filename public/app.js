...
testElastic.controller("testController", function($scope, $http, $timeout, $q) {
    ...
    $scope.selectedAttributes = [];
    var range = [];
    $scope.selectedAttribute = function(attributes, value, selected) {
      var attributeSelected = {};
      var select = true;
      
        var from = value.from || 0;
        var to = value.to || 100000;
      
      var shouldQuery = [];
      if(attributes.name == "Price") {
      if(selected) {
      ...
      {  
        range.splice(range.indexOf(from),1);
        range.splice(range.indexOf(to),1); 
      }
     }
     var attribute = attributes.name.replace(/\s/g,"").toLowerCase();
      if(attribute == "brand") {
        attribute = attribute + ".org";
      }
      ...
      {
        attribute = "attribute."+attribute+".org";
      }
      if(select)
      attributeSelected[attribute] = [value.key];
      if(range.length>0) 
      {   console.log("range is ", range);
          var max = _.max(range);
          var min = _.min(range);
          var shouldQueryObject  = {};
          shouldQueryObject["range"]  = {
            "price": {
              "gte": min,
              "lte" : max  
            }
          };
          shouldQuery.push(shouldQueryObject);
      }
      else
      {
        var shouldQueryObject  = {};
      ...
      $scope.selectedAttributes.forEach(function(data) {
          var shouldQueryObject = {};
          shouldQueryObject["terms"] = data;
          shouldQuery.push(shouldQueryObject);
      });
      
      console.log("shouldQuery is :", shouldQuery);
      query = {
          "query" : {
            "filtered": {
              "query": {
                "match": {
                  "category.org": $scope.subresult
                }
              },
              "filter": {
                "bool": {
